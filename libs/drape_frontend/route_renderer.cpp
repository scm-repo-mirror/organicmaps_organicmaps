...
void InterpolateByZoom(SubrouteConstPtr const & subroute, ScreenBase const & screen, float & halfWidth, double & zoom)
{
  ...
  switch (subroute->m_routeType)
  {
    ...
    case RouteType::Bicycle:
      halfWidthInPixel = &kRouteHalfWidthInPixelBicycle;
      break;
    case RouteType::Transit:
      halfWidthInPixel = &kRouteHalfWidthInPixelTransit;
      break;
    case RouteType::Pedestrian:
    case RouteType::Ruler:
      halfWidthInPixel = &kRouteHalfWidthInPixelOthers;
      break;
  }
                                    ...
                                    ref_ptr<gpu::ProgramManager> mng)
{
  ...
  else
  {
    ...
    auto const it = std::upper_bound(m_subroutes.begin(), m_subroutes.end(), info, [](SubrouteInfo const & info1, SubrouteInfo const & info2)
